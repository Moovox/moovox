...
const vaccineService = {
  ...
  async createVaccine(data) {
    try {
      ...
      if (
        typeof data.target_disease !== "string" ||
        data.target_disease.trim().length === 0
      ) {
        throw new Error(
          "O campo 'target_disease' deve ser uma string não vazia."
        );
      }
      ...
      if (expiration_date < new Date()) {
        throw new Error(
          "O campo 'expiration_date' não pode ser uma data passada."
        );
      }
      ...
      if (
        data.dosing_interval !== undefined &&
        isNaN(Number(data.dosing_interval))
      ) {
        throw new Error(
          "O campo 'dosing_interval' deve ser um número, se fornecido."
        );
      }
  ...
  async getAllVaccineTypes() {
    try {
      const types = await prisma.types_of_Vaccines.findMany({
        select: {
          id: true,
          name: true,
          description: true,
