...
// Function to get species name in English
const getSpeciesName = (species) => {
  // Return species name as is (in English)
...
const animalService = {
  async getAllAnimals(farmId) {
    try {
      ...
      return animals.map((animal) => ({
        id: animal.id,
        identification: `${animal.species.name
          .substring(0, 3)
          .toUpperCase()}-${animal.id.toString().padStart(3, "0")}`,
        name: animal.name,
        species: getSpeciesName(animal.species.name),
  ...
  async createAnimal(data) {
    try {
      ...
      if (!farm) {
        throw new Error(
          `A fazenda com ID ${farmId} não existe. Selecione uma fazenda válida.`
        );
      }
  ...
  async updateAnimal(id, data) {
    try {
      ...
      if (!existingAnimal) {
        throw new Error(
          "Animal não encontrado ou não pertence à fazenda especificada"
        );
      }
