...
const vaccineApplicationController = {
  async getAllVaccineApplications(req, res) {
    try {
      const vaccine_application =
        await vaccineApplicationService.getAllVaccineApplications();
      res.status(200).json({
        status: "success",
  ...
  async getVaccineApplicationByID(req, res) {
    try {
      const { id } = req.params;
      const user = await vaccineApplicationService.getVaccineApplicationByID(
        id
      );
      res.status(200).json({
        status: "success",
      ...
      res.status(500).json({
        status: "error",
        message:
          "Ocorreu um problema ao buscar o usuário. Por favor, tente novamente mais tarde.",
      });
    }
  },
  async createVaccineApplication(req, res) {
    try {
      const newVaccineApplication =
        await vaccineApplicationService.createVaccineApplication(req.body);
      res.status(201).json({
        status: "success",
      ...
      res.status(500).json({
        status: "error",
        message:
          "Ocorreu um problema ao criar o animal. Por favor, tente novamente mais tarde.",
      });
    }
  },
};
