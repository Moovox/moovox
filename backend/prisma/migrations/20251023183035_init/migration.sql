...
CREATE TABLE `Applications` (
    ...
    `status_vaccine_application` VARCHAR(191) NOT NULL,
    `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updated_at` DATETIME(3) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `Farmhands` ADD CONSTRAINT `Farmhands_user_id_fkey` FOREIGN KEY (`user_id`) REFERENCES `Users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Applications` ADD CONSTRAINT `Applications_animal_id_fkey` FOREIGN KEY (`animal_id`) REFERENCES `Animals`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Locations` ADD CONSTRAINT `Locations_animal_id_fkey` FOREIGN KEY (`animal_id`) REFERENCES `Animals`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
