...
export const animalService = {
    listAnimals: async () => {
        try {
            ...
            if (!farmId) {
                console.warn('Warning: Farm ID not found in localStorage. Animals may not be filtered correctly.');
            }
        ...
        try {
            ...
            if (error.response) {
                ...
                if (error.response.status === 400 && error.response.data?.code === 'FARM_ERROR') {
                    ...
                    throw new Error(error.response.data.message || 'The selected farm does not exist or is not available.');
                }
                ...
                // Other server errors
                throw error.response.data || new Error('Error processing request on server.');
