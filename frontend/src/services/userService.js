...
export const userService = {
    ...
    async removeFarmhandRole(id) {
        try {
            ...
            if (response.status === 200) {
                return { 
                    success: true, 
                    message: response.data.message || 'Farmhand role removed successfully'
                };
            }
    ...
    async removeVeterinarianRole(id) {
        try {
            ...
            if (response.status === 200) {
                return { 
                    success: true, 
                    message: response.data.message || 'Veterinarian role removed successfully'
                };
            }
        ...
        try {
            ...
            if (response.status === 200) {
                return { 
                    ...
                    message: response.data.message || 'Applications transferred successfully'
                };
            }
    ...
    async getAllUsers() {
        try {
            ...
            if (!farmId) {
                console.warn('Warning: Farm ID not found in localStorage. Users may not be filtered correctly.');
            }
            ...
            const users = usersData.map(user => ({
                ...
                email: user.email,
                type: typeMap[user.type || user.role || user.tipo] || user.type || user.role || user.tipo,
