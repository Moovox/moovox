...
function AnimalEditModal({ animal, open, onOpenChange, onSuccess }) {
    ...
    useEffect(() => {
        if (animal && open) {
            ...
            const speciesItem = animalService.getSpecies().find(
                s => s.label.toLowerCase() === animal.species.toLowerCase()
            );
    ...
    useEffect(() => {
        if (formData.speciesId) {
            const breedList = animalService.getBreedsBySpecies(parseInt(formData.speciesId));
            setBreeds(breedList);
            
            // Reset breed if it's not in the new list of breeds
            const breedExists = breedList.some(b => b.id.toString() === formData.breedId);
            if (!breedExists) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast({
                ...
                description: error.message || 'An error occurred while updating the animal',
                variant: "destructive"
            });
        } finally {
            setLoading(false);
