...
function UserCreateModal({ onSuccess }) {
  ...
  const validateForm = () => {
    ...
    if (!currentFarmId) {
      newErrors.farm =
        "No farm selected. Please select a farm before creating a user.";
    }
    ...
    try {
      ...
      if (error.message.includes("already exists")) {
        ...
        toast({
          title: "Farm error",
          description:
            error.message || "There is a problem with the selected farm",
          variant: "destructive",
        });
      } else {
        toast({
  ...
  return (
    ...
    >
      ...
      {farmInfo && (
        <div className="mb-4 rounded border border-amber-200 bg-amber-50 p-2 text-sm text-amber-800">
