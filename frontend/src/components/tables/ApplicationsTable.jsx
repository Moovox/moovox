...
function ApplicationsTable({ applications, loading, error, onApplicationCreated }) {
    ...
    const filteredApplications = applications?.filter(application =>
        (application?.animalName?.toLowerCase().includes(search.toLowerCase()) || 
         application?.animalIdentification?.toLowerCase().includes(search.toLowerCase()) || 
         application?.vaccineName?.toLowerCase().includes(search.toLowerCase()) || 
    ...
    const handleDelete = async (id, animalName, vaccineName) => {
        ...
        if (!window.confirm(`Are you sure you want to delete the application of "${vaccineName}" to "${animalName}"?`)) {
            return;
        }
        ...
        try {
            ...
            toast({
                ...
                description: error.message || 'An error occurred while deleting the application',
                variant: "destructive"
            });
        } finally {
                ...
                <TableRow>
                    <TableCell colSpan={isMobile ? 4 : 6} className="h-24">
                        <Alert variant="destructive" className="flex items-center gap-2">
                            ...
                            <AlertDescription>
                                {error.message || 'Error loading applications. Please try again.'}
                            </AlertDescription>
                        </Alert>
                    </TableCell>
                </TableRow>
        ...
        return paginatedApplications.map((application) => {
            ...
            return (
                <TableRow key={application.id}>
                    ...
                    <TableCell>{applicationDate ? applicationDate.toLocaleDateString() : '-'}</TableCell>
                    <TableCell>
                        <div className="font-medium">{application.animalName}</div>
                        <div className="text-xs text-gray-500">{application.animalIdentification}</div>
                    </TableCell>
                    ...
                    <TableCell>
                        <div className="flex justify-center gap-2">
                            ...
                            <Button 
                                ...
                                title="Delete"
                                onClick={() => handleDelete(application.id, application.animalName, application.vaccineName)}
